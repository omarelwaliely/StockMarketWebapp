...
Response Register::execute(const Request& request) {
    ...
    try {
        ...
        if (users.find(email) != users.end()) { //theres a collision the email exists, since its the primary key we cant allow that
            ...
            Response res("encryptedsecretkey", body.dump(), 500);
            return res;
        }
        ...
        Response res("encryptedsecretkey", body.dump(), 300);
        return res;
    }
    ...
    Response res("encryptedsecretkey", body.dump(),200);  //we have a successful response
    return res;
}
